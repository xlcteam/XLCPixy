...
public:
  ...
  uint16_t getBlocks(uint16_t maxBlocks=XLCPIXY_BLOCKS_COUNT, uint32_t maxDeltaTime=XLCPIXY_MAX_NO_DATA_TIME);
...
template <class LinkType> boolean XLCPixy<LinkType>::getStart(uint32_t maxNoDataTime)
{
  uint16_t w, lastw;
  uint32_t noDataTime;
  ...
  while(true)
  {
    ...
    if (w == 0)
    {
      ...
      else if (millis() - noDataTime > maxNoDataTime) // still getting no data
      {
...
template <class LinkType> uint16_t XLCPixy<LinkType>::getBlocks(uint16_t maxBlocks, uint32_t maxNoDataTime)
{
  uint8_t i;
...
template <class LinkType> int8_t XLCPixy<LinkType>::setServos(uint16_t s0, uint16_t s1)
{
  uint8_t outBuf[6];
...
template <class LinkType> int8_t XLCPixy<LinkType>::setBrightness(uint8_t brightness)
{
  uint8_t outBuf[3];
...
template <class LinkType> int8_t XLCPixy<LinkType>::setLED(uint8_t r, uint8_t g, uint8_t b)
{
  uint8_t outBuf[5];
